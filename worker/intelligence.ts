...
type PlayerContext = { depthRank?: number; depthSlot?: string; depthDate?: string; rosterStatus?: string; game?: GameContext };
...
async function optionalCsv(url: string): Promise<CsvRow[]> { try { return await fetchCsv(url); } catch { return []; } }
...
function displayName(row: CsvRow) { return row.player_display_name || row.player_name || row.full_name || row.display_name || row.name || ''; }
...
function fantasyPoints(row: CsvRow, position: string) {
  ...
  if (['DL','DE','DT','LB','DB','CB','S'].includes(pos)) {
    return num(row,'def_tackles_combined','tackles_combined','tackles') * 0.75 + num(row,'def_sacks','sacks') * 3 + num(row,'def_interceptions','interceptions') * 3 + num(row,'def_fumbles_recovered','fumble_recoveries') * 2 + num(row,'def_tds','defensive_tds') * 6;
  }
  if (pos === 'K') return num(row,'fg_made','field_goals_made') * 3 + num(row,'pat_made','extra_points_made');
  return direct;
}
...
function buildSamples(rows: CsvRow[], player: RosterPlayer): WeeklySample[] {
  ...
  return rows.filter(row => normalizeName(displayName(row)) === key)
    ...
    .map(row => ({ week:num(row,'week'), points:fantasyPoints(row,pos), team:row.recent_team || row.team || row.team_abbr }))
    .filter(sample => Number.isFinite(sample.points));
}

function depthContext(player: RosterPlayer, rows: CsvRow[]): Pick<PlayerContext,'depthRank'|'depthSlot'|'depthDate'> {
  const name = normalizeName(player.name ?? '');
  const team = (player.nflTeam ?? '').toUpperCase();
  const matches = rows.filter(r => normalizeName(displayName(r)) === name && (!team || !r.team || r.team.toUpperCase() === team));
  matches.sort((a,b) => String(b.dt || '').localeCompare(String(a.dt || '')));
  const row = matches[0];
  if (!row) return {};
  const rank = num(row,'pos_rank','depth_team');
  return { depthRank: rank || undefined, depthSlot: row.pos_slot || row.pos_name || row.depth_position || undefined, depthDate: row.dt || undefined };
}

function nextGame(team: string, rows: CsvRow[]): GameContext | undefined {
  if (!team) return undefined;
  const today = new Date().toISOString().slice(0,10);
  const candidates = rows.filter(r => String(r.season) === '2026' && (!r.game_type || r.game_type === 'REG'))
    .filter(r => r.home_team === team || r.away_team === team)
    .filter(r => (r.gameday || r.game_date || '') >= today)
    .sort((a,b) => String(a.gameday || a.game_date || '').localeCompare(String(b.gameday || b.game_date || '')));
  const row = candidates[0];
  if (!row) return undefined;
  const home = row.home_team === team;
  return { opponent: home ? row.away_team : row.home_team, gameDate: row.gameday || row.game_date, homeAway: home ? 'home' : 'away' };
}

function roleMultiplier(position: string, rank?: number) {
  if (!rank || rank <= 1) return 1;
  if (position === 'QB') return rank === 2 ? 0.35 : 0.1;
  if (['RB','WR','TE'].includes(position)) return rank === 2 ? 0.84 : rank === 3 ? 0.64 : 0.5;
  if (['DL','DE','DT','LB','DB','CB','S'].includes(position)) return rank === 2 ? 0.78 : 0.58;
  return rank === 2 ? 0.8 : 0.6;
}

function unavailableStatus(status: string) {
  const s = status.toLowerCase();
  return /injured reserve|\bir\b|pup|suspend|reserve\/|non-football injury|out/.test(s);
}

function projectPlayer(player: RosterPlayer, stats2025: CsvRow[], stats2026: CsvRow[], currentRoster: CsvRow[], depthRows: CsvRow[], scheduleRows: CsvRow[]): Projection {
  ...
  if (rosterMatch) {
    ...
    if (currentTeam && team && currentTeam !== team) { confidence -= 0.08; reasons.push(`Current public roster data lists ${currentTeam}, while RTSports lists ${team}; role/team transition lowers confidence.`); }
    else reasons.push('Current public roster context matches the synchronized RTSports player identity/team.');
    rosterStatus = rosterMatch.status_description || rosterMatch.status || '';
    if (rosterStatus) reasons.push(`Current roster status: ${rosterStatus}.`);
  } else reasons.push('No current nflverse roster match was found; availability context is incomplete.');

  const depth = depthContext(player, depthRows);
  if (depth.depthRank) {
    const multiplier = roleMultiplier(position, depth.depthRank);
    if (multiplier !== 1 && baseline > 0) baseline *= multiplier;
    confidence += depth.depthRank === 1 ? 0.06 : -0.03;
    reasons.push(`Latest public depth chart lists ${name} at ${depth.depthSlot ?? position} rank ${depth.depthRank}${depth.depthDate ? ` as of ${depth.depthDate}` : ''}.`);
    if (multiplier < 1) reasons.push(`Role adjustment applies a ${Math.round((1-multiplier)*100)}% opportunity discount for current depth position.`);
  } else reasons.push('No current depth-chart rank was matched; role confidence is unchanged.');

  const game = nextGame(team, scheduleRows);
  if (game?.opponent) reasons.push(`Next scheduled opponent is ${game.opponent}${game.gameDate ? ` on ${game.gameDate}` : ''} (${game.homeAway}).`);

  if (rosterStatus && unavailableStatus(rosterStatus)) {
    baseline = 0;
    confidence = Math.max(confidence,0.9);
    reasons.push('Current roster status indicates the player is unavailable, so the active-week projection is zeroed until status changes.');
  }

  if (position === 'HC') { baseline = 0; confidence = 0.15; reasons.push('Head-coach scoring requires league scoring rules and team-result modeling; no generic projection is published yet.'); }

  const projection = baseline > 0 ? Number(baseline.toFixed(2)) : null;
  return { playerKey:playerKey(name,position), name, position, team, projection, confidence:Math.max(0.05,Math.min(0.95,Number(confidence.toFixed(2)))), trend:trendFor(current.length >= 4 ? current : historical), reasons, sourceGames:historical.length + current.length, depthRank:depth.depthRank, opponent:game?.opponent };
}
...
export async function refreshIntelligence(env: IntelligenceEnv) {
  ...
  try {
    ...
    const [stats2025,stats2026,roster2026,depth2026,schedule] = await Promise.all([
      ...
      optionalCsv(SCHEDULE_URL),
    ]);
    const projections = roster.map(player => projectPlayer(player,stats2025,stats2026,roster2026,depth2026,schedule));
    ...
    for (const p of projections) {
      statements.push(env.DB.prepare(`INSERT INTO player_intelligence
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ON CONFLICT(player_key) DO UPDATE SET display_name=excluded.display_name, position=excluded.position, nfl_team=excluded.nfl_team, projection=excluded.projection, confidence=excluded.confidence, trend=excluded.trend, reasons_json=excluded.reasons_json, source_games=excluded.source_games, model_version=excluded.model_version, updated_at=excluded.updated_at`)
    ...
    const sourceSummary = { stats2025Rows:stats2025.length, stats2026Rows:stats2026.length, roster2026Rows:roster2026.length, depth2026Rows:depth2026.length, scheduleRows:schedule.length, modelVersion:MODEL_VERSION };
    await env.DB.prepare(`UPDATE intelligence_runs SET completed_at=?, status='success', source_summary=?, player_count=?, message=? WHERE id=?`).bind(now,JSON.stringify(sourceSummary),projections.length,'Projection refresh completed.',runId).run();
    return { ok:true, runId, playerCount:projections.length, sourceSummary, updatedAt:now };
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    await env.DB.prepare(`UPDATE intelligence_runs SET completed_at=?, status='failed', message=? WHERE id=?`).bind(new Date().toISOString(),message,runId).run();
...
export async function getLatestIntelligence(db:D1Database) {
  ...
  return { run:run ? { ...run, source_summary:run.source_summary ? JSON.parse(String(run.source_summary)) : null } : null, players:result.results.map(row => ({ playerKey:row.player_key, name:row.display_name, position:row.position, team:row.nfl_team, projection:row.projection, confidence:row.confidence, trend:row.trend, reasons:JSON.parse(String(row.reasons_json ?? '[]')), sourceGames:row.source_games, modelVersion:row.model_version, updatedAt:row.updated_at })) };
}
