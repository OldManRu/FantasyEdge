...
type ConfigPayload = { schemaVersion?:number; source?:string; deviceId?:string; pageUrl?:string; syncedAt?:string; pageTitle?:string|null; leagueName?:string|null; season?:number|null; sections?:unknown[]; rawText?:string };
...
export default {
 ...
 async scheduled(_controller:ScheduledController,env:Env,ctx:ExecutionContext):Promise<void>{if(!env.DB)return;ctx.waitUntil(refreshWithSignals(env.DB));}
