...
type BackendAlternative = { kind: 'safer-floor' | 'upside' | 'league-history'; label: string; slot: string; start: Player; bench: Player; projectedDelta: number; confidence: number; explanation: string };
...
export default function App() {
  ...
  const alternatives = backendReady ? (backendLineup?.alternatives ?? []) : [];
  ...
  return <main className="shell">
    ...
    {alternatives.length ? <section className="alternative-angles"><div className="section-title"><div><p className="label">Strategic alternatives</p><h2>Alternative Angles</h2></div><span className="pill">Legal lineup options · not the primary pick</span></div><p className="alternative-intro">Fantasy Edge still recommends the highest expected-points lineup first. These are legal alternatives when you prefer a different risk profile.</p><div className="alternative-grid">{alternatives.map((alt,i) => <article className={`alternative-card ${alt.kind}`} key={`${alt.kind}-${alt.slot}-${alt.start?.name}-${i}`}><div className="alternative-head"><div><span className="alternative-kind">{alt.label}</span><strong>Start {alt.start?.name ?? 'Unknown'} over {alt.bench?.name ?? 'Unknown'}</strong></div><b className={alt.projectedDelta >= 0 ? 'positive' : 'negative'}>{alt.projectedDelta >= 0 ? '+' : ''}{fmt(alt.projectedDelta)} pts</b></div><div className="alternative-meta"><span>{alt.slot}</span><span>{pct(alt.confidence)} confidence</span></div><p>{alt.explanation}</p></article>)}</div></section> : null}
    ...
    <section className="coming-next"><p className="label">Intelligence pipeline</p><div className="roadmap"><span>✓ Roster sync</span><span>✓ Historical performance</span><span>✓ Projection snapshots</span><span>✓ Scheduled refresh</span><span>✓ Depth charts</span><span>✓ Matchup strength</span><span>✓ Injury reports</span><span>✓ News signals</span><span>✓ Backend optimizer</span><span>✓ Alternative angles</span><span>League scoring import</span><span>Waiver recommendations</span></div></section>
  </main>;
}
