...
export default function App() {
  ...
  const recommendationSource = backendReady ? (backendLineup?.rulesSource === 'imported' ? 'RTSports league rules' : 'Fantasy Edge fallback rules') : 'frontend fallback';
  ...
  return <main className="shell">
    ...
    {latest && insights && <section className="insights"><div className="section-title"><div><p className="label">This week</p><h2>Action Center</h2></div><span className="pill">{backendReady ? `Backend optimizer · ${backendLineup?.rulesSource === 'imported' ? 'league rules' : 'fallback rules'}` : modelReady ? 'Fantasy Edge model active' : 'Building Fantasy Edge model'}</span></div><div className="insight-grid">
      ...
      <article className={`insight-card ${backendMoves.length || insights.fallbackMoves.length ? 'attention' : 'neutral'}`}><span className="insight-kicker">Optimizer</span><strong>{backendReady ? `${backendMoves.length} recommended change${backendMoves.length === 1 ? '' : 's'}` : insights.fallbackMoves.length ? `${insights.fallbackMoves.length} fallback upgrade${insights.fallbackMoves.length === 1 ? '' : 's'}` : modelReady ? 'Current starters lead' : 'Model refresh pending'}</strong><p>{backendReady ? `Authoritative backend lineup: ${fmt(Number(backendLineup?.projectedPoints ?? 0))} projected points using ${backendLineup?.rulesSource === 'imported' ? 'imported league rules' : 'temporary fallback rules'}.` : fallbackTop ? `${fallbackTop.player.name} projects ${fmt(fallbackTop.edge)} above ${fallbackTop.replace.name}.` : modelReady ? 'No modeled bench player currently grades above a same-position starter.' : 'A projection refresh will run automatically after roster sync or on the scheduled data cycle.'}</p></article>
